...
**[下载 v0.1 APK](https://github.com/abdvl/olevod-tv-app/releases/download/v0.1/olevod-tv-v0.1.apk)** · **[发布说明与校验文件](https://github.com/abdvl/olevod-tv-app/releases/tag/v0.1)** · **[完整安装教程](docs/INSTALL.md)**

| 项目 | v0.1 |
| --- | --- |
| 应用版本 | 0.1.0 · versionCode 1 |
| 安装包 | `olevod-tv-v0.1.apk`，约 14.2 MB |
...
- **VIP 需要网站有效会员。** “VIP 蓝光”是网站分类名称，不代表每个片源都是 4K/HDR。实际清晰度、音轨与播放能力取决于片源、会员权限、网络和设备解码支持；4K/HDR 与至少 30 分钟连续播放的完整验收尚未完成。
...
记录每次 release 开发所用的模型、token 用量，以及按记录时 OpenAI API 公开价格折算的美元价值。**这是开发成本估算，不是 Codex 订阅账单或实际付款金额。**

| Release | 模型 | 输入 token（含缓存） | 其中缓存输入 | 输出 token（含推理） | 总 token | 标准 API 估值（USD） |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
...
后续 release 在表格中追加一行，记录该版本相对上一统计截止点的新增用量，避免重复累计；同时保存统计范围、截止时间及当时单价。若使用多个模型或计费档位，应分别计算后汇总；缺失数据标为“未统计”，不要填 0。历史估值保留当时价格，不随未来调价覆盖。
