...
| R06 | 分类导航进入小首页：人气、评分各前十二，各榜前二突出、后十张五列两行，最后“浏览全部”；VIP使用全部年份，其他分类当年。（2026-09-08开发后修订） |
...
2. 内容顺序：`年份范围 人气最高` → `年份范围 评分最高` → `浏览全部`。
3. 两榜按 `sort=hot/score` 获取前十二个有效结果，不在客户端自行重排。普通分类使用 `year=当前年份`；VIP作为片库使用 `year=0`，标题明确“全部年份”，避免当年无VIP片时出现空白。普通分类年份由设备本地日期生成，测试注入固定时钟；跨年重新进入时更新缓存键。
4. 各榜前两名在该榜最上方单独突出，用两个等宽“完整竖海报＋旁侧排名／片名／元信息”卡，不把前二重复放进后续十张。背景使用该片海报的模糊副本拉伸铺满圆角卡片，形成随影片变化的柔和底色；前景海报仍保持完整Fit，文字不参与模糊。文字起点前就达到至少72%的深色遮罩，右侧82%，底部再渐暗；背景无独立焦点和朗读内容，图片失败回退Panel。模糊使用小尺寸缓存图，不随焦点移动重新计算，并兼容最低API26。（2026-09-08背景修订）
5. 第三至十二名用竖版 `PosterTile`，五列×两行；本页可自然长滚动，不要求整榜首屏完整显示。人气榜和评分榜允许同一影片各出现一次。
6. 不足十二部展示实际数量，保留正确排名。普通分类不用往年片补齐“当年”榜单；为空时写“今年暂无相关影片”。VIP为空写“暂无相关影片”。始终提供浏览全部。
...
- 目录网格六列，间距 12 dp；每张基准宽 138 dp、竖图高 207 dp；图外最多两行片名、一行元信息。共享海报组件将有效评分叠加在右上角，距边6dp、13sp绿色粗体、半透明深色圆角底，评分缺失时省略且不单独占焦点。片名使用海报下方全宽，不再与评分挤占空间；海报保持等比Fit，不改变裁切方式。（2026-09-08开发后修订）
