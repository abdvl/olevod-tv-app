...
### S57 - UI v2 Chromecast core regression and VIP source finding

- Discovered the Chromecast's current wireless connection port through native Bonjour and reused the existing pairing. Independently verified sabrina / Android14 / 32-bit ARM, 1920×1080 override and density320. Matched the old/new debug certificate, backed up only the old APK locally, and installed the unchanged UI v2 App with `install -r`, retaining account/history data.
- Independent real-device runs passed 45 UI/state cases (225.692s), live catalog (9.597s), and ordinary first-frame/format/progress (19.508s), with no skips. Added opt-in `V2DevicePlaybackTest`; independent review fixed a paused-platform-speed assumption before execution. First run exposed a test-only Int return causing JUnit initialization failure; explicit Unit corrected it and the second run passed (29.889s), preserving both logs.
- The new actual-media case verified 30/300-second seeks and boundary clamping against platform positions, 1.5 playback speed, unchanged fullscreen video-container bounds/session/paused position, Activity ON_STOP pause, retained position on return, isolated history save and session release. No production code or App APK changed.
- Saved-account VIP check failed twice: ID80632 returned one episode with an empty URL. Improved only the diagnostic assertion to report scheme counts without URLs; kept the HTTPS assertion. Favorites/cloud-history checks after it did not execute. This is an unresolved source/account-contract finding, not evidence of decoder failure or verified VIP playback.
- Actual MN→魔女→first result→Back to input screenshots and the real catalog view are archived as four reviewed images. Kept existing personal history/account screenshots local. Later manual page state diverged from the intended key sequence; paused device keys and asked about concurrent remote use, without claiming that segment passed.
- Reports: `docs/verification/v2/CHROMECAST_V2.md` and `CHROMECAST_INDEPENDENT.md`. Core result is 48 passed / 1 VIP check failed, plus the separately retained test-initialization first attempt. Real audio/sync, 4K/HDR, 30-minute playback, system Home/process recreation and full accessibility remain unverified. No version bump or release publication.

### S58a - diagnose and repair expired VIP playback sessions

- Two separately gated account reads first failed with the website's explicit expired-login response. Reusing the TV's remembered credentials and the previously authorized CAPTCHA flow restored the session; the independent verifier then passed both reads (4.569s). VIP80632 returned one HTTPS base source, no empty address and no premium variants; favorites and cloud history were actually read. No credentials or CAPTCHA values were logged.
- Public VIP detail can return code0 with all base episode addresses blank for an expired session. `OlevodApi.detail` now diagnoses that restricted, nonempty shape: guest login guidance, server-confirmed expiry, missing/invalid membership data, entitlement failure and unavailable source have distinct outcomes. Healthy/mixed sources and empty episode lists keep their normal request path. Responses from an account that changed during loading are discarded.
- Independent new JVM tests first exposed one stale-error race (13/14 passed); the catch guard fixed it without weakening assertions. Final new15/15, entire JVM45/45, lint and App/Test builds pass. App SHA-256 `2af5e7a174200a90d70f21f3b0808b58f73d27fc3d2631d2c0b999592793f3ae` was installed with `install -r`, preserving data. Production player/UI code is unchanged.
- Follow-up device regression, explicit VIP decoding and30-minute playback are in progress; this checkpoint does not claim those completed. Detailed first/second test attempts are retained under `.tools/chromecast-v2/`.

### S58b - post-fix device regression and VIP decoding

- Independent post-fix Chromecast regression passed16/16 in112.443s (account reads2, ordinary native playback1, Player UI4, Root journey3, Account UI6). The new read-only `V2VipDecodeTest` passed1/1 in15.451s: first frame,1920×1040,0→4770ms,115 video output buffers and242 audio output buffers. This is decoding evidence, not audible sound/sync confirmation. App remains2af5e7a; Test APK8dd48499. No account/history writes in the new decoder test.
- Real D-pad catalog Score selection and traversal through rows5/6 with return to5 passed; deep posters and metadata fit. Sorting can retain a deep viewport until entering results, retained as a follow-up finding. Actual systemHome/return on the preceding App paused at379879ms and retained exactly that position.
- Added reusable explicit-device `scripts/chromecast-soak.py`. Its6-second preflight captured actual player-clock advancement and parsed platform/foreground/memory/crash state. A30-minute actual-app run started02:28 UTC, logging to `.tools/chromecast-v2/soak-30min.jsonl`; do not restart/install/interact with the Chromecast until this run ends. Long-run acceptance remains in progress. Error-action focus is being checked separately on the emulator.

### S58c - make playback recovery actions reachable by remote

- Independent production-component tests reproduced4/4 failures: visible login/retry buttons could not be reached through the explicit video/header focus route. PlayerContent now routes controlUp and HeaderDown to the primary recovery action, links login/retry/controls vertically, returns focus before retry removes its button, preserves recovery while fullscreen, and does not steal Header focus when an error arrives. Healthy-video controls and media pipeline are unchanged.
- Independent emulator verification: original4 failure assertions now4/4 pass (6.907s), existing Player4+Root3 pass7/7 (19.670s), and fontScale1.3 targeted4/4 pass (10.026s). Recovery button bounds fit the screen and remain above controls; fontScale was restored. Synthetic screenshots and all failure logs retained; these emulator results do not replace the pending physical-device rerun.
- Corrected fullscreen error hint to "返回键退出全屏"; final independent assembleDebug/assembleDebugAndroidTest/lintDebug/JVM run succeeds. JVM45/45, lint0errors/21warnings. Final App SHA `ac0ca731f8d721f36e1baed06ca7325587407d21a399299d3d224976f2acdc4f`; Test SHA `f983fffe7e29792e864fd6feab5b9be565aed8e8089e18eeee7f6c302a5c55fb`. It is not yet installed on Chromecast: the ongoing30-minute observation remains on2af5e7a.

### S58d - reset a changed catalog query after its first results arrive
...
### S62 - make failed first-page searches recoverable by remote

- During fresh release screenshots, a real transient search failure exposed an unreachable results retry button and same-term confirmation that did not reload the cached failed feed. Independent controlled tests reproduced both failures (2/2,11.077s) before changing production.
- First-page error results now have an explicit focus target reachable from the middle suggestions/retry column or keyboard. Confirming retry first returns focus to the attached input before the button disappears, then moves to the first result on success. Confirming the same failed suggestion starts a new request. Left and Back preserve the documented return routes.
- Pending results-entry navigation is a cancellable job. A new key press, Back, edited/confirmed query or feed disposal cancels an old frame-waiting focus transfer. Independent review identified this narrow race; a paused Compose frame regression verifies that Right followed immediately byBack cannot steal input focus.
- Final debugApp9babf76b2d166ff9538cc76c07a6cf0a607d257f80631e0dc8b7823ca3361926; Teste06015807c11ba4d2620cdba8f8a7017156697aeb5f051ffbecb1400889f027d. Final Search7/7 pass17.855s; build/JVM47/lint pass. Old2 failures and first fixed6 passes are retained in `verification/v2/SEARCH_RETRY.md` and local logs.
- User's README rewrite/token-history/push/release request remains active. Product documentation and new screenshots are prepared separately; token accounting is frozen before this release request and excludes this follow-up fix. New release must include this search fix; precedingc43c2569 candidate is superseded, as is archived001c33cf. No GitHub publication yet.

### S63 - refresh product documentation, screenshots and release accounting

- Rewrote README around choosing a film, continuing playback, remote search, grouped episodes and remembered login. Moved detailed engineering and accounting methods behind document links; installation and release notes match the new interface. Added a12-page screenshot gallery and index under `screenshots/v0.2/`. Each actual signed-release screenshot was independently viewed; no real account details appear. Per-image APK/source hashes are in the manifest, including earlier candidate images for unchanged pages.
- Added incremental working-model accounting from the prior design cutoff through2026-09-08 21:18:45.833 America/Los_Angeles:1,348 requests, input188,690,723 (cached184,708,864), output661,464, total189,352,187; Standard API equivalent$257.600654, displayed$257.60. Work-model project totals now363,070,050 tokens/$485.33. Platform auto-review55,959,929 tokens are separately disclosed with unknown cost. Current README/screenshot/accounting/final-publication work and S62 are after this frozen cutoff, not silently included.
- Preserved originalv0.1 assets and both supersededv0.2 candidates. Final package from production1d10dbf is `artifacts/releases/v0.2/olevod-tv-v0.2.apk`, SHA-256 `3018708ebb6b1e73d62f7f91fa56b3b0f7dd3c86215e5e923dd3f703108141c5`,14,341,714 bytes, version0.2.0/code2, originalv0.1 release certificate. Release build/JVM47/lint0errors21warnings pass; checksum file independently verified. Dedicated TV emulator results belong to the precise APK and scope in the final report, not a new Chromecast release installation.
...
### S65 - preserve README reference material around the product introduction

- User clarified that only the feature introduction should receive product copy; other README content must stay. Restored the original project/technical introduction, version table, full16-row remote reference, installation and migration detail, validation scope, developer instructions, complete document index, and historical token accounting explanations from the pre-rewrite README.
- Kept the new product feature copy and12 current screenshots in the feature section. Retained currentv0.2 release links,47 JVM result, incremental accounting numbers/cutoff and published status; historical feature galleries remain linked by version. No application, APK, release tag or accounting cutoff was changed.
- Checked all local README links and12 image paths, preservation of all six non-feature sections and the full documentation table, and exact retention of the oldv0.1/design accounting details. Documentation-only change; no application test rerun needed.
- This README correction is committed locally but not yet pushed: the Mac locked again, and the actual noninteractive main push failed with `fatal: unable to get password from user`. Resume by pushing the existing main commit through GitHub Desktop after unlock, then verify the remote SHA. The v0.2 release itself remains published; no new release or tag is needed for this documentation fix.
