...
这是**可运行测试版**，不是所有首版需求均已验收。目标设备：Google Chromecast with Google TV（4K/HD未确认）。

| 步骤 | 当前状态 |
| --- | --- |
| S01–S06 | 完成：工程/界面/API与真实首页 |
| S07–S08 | 已接通：真实筛选、搜索；独立验证美国筛选与热词结果，更多排序/分页边界继续回归 |
| S09 | 普通点播与控制已独立通过；VIP待账号 |
| S10 | 电视登录/Keystore代码完成；真实登录等待新鲜验证码 |
| S11–S12 | 直播目录/节目单/回看代码完成；游客媒体子清单错误，直播/回看播放未通过 |
| S13 | 本机点播历史25条、重开与账号隔离测试通过；UI续播复测中 |
| S14 | 影视与频道收藏代码完成；账号实际增删与列表待验收 |
| S15 | 网站本地历史/同步协议边界已记录；云历史读取/同步待账号真实响应 |
| S16 | 焦点、后台播放、账号竞争、异常恢复已修，修复版独立回归中 |
| S17 | build/lint/13协议测试通过；最终报告与测试包整理中，Chromecast未连接 |

- 当前用户交互依赖：登录验证码未回复；Mac锁定导致无法访问已登录Chrome。未自动绕过验证码。
- 下一步：独立agent复测修复版；解锁/验证码可用后优先登录、VIP、直播回看与收藏，再确认云历史范围。
