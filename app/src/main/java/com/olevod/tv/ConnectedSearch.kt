...
fun ConnectedSearch(vm:AppViewModel,open:(Movie)->Unit) {
    ...
    Row(Modifier.fillMaxSize().padding(40.dp,12.dp,40.dp,20.dp),horizontalArrangement=Arrangement.spacedBy(30.dp)){
        ...
        Column(Modifier.weight(1f),verticalArrangement=Arrangement.spacedBy(12.dp)){
            ...
            when {
                query.isBlank()->LazyColumn(verticalArrangement=Arrangement.spacedBy(8.dp)){items(hot.chunked(2)){row->Row{row.forEach{word->TvAction(word,modifier=Modifier.weight(1f)){query=word}};repeat(2-row.size){Spacer(Modifier.weight(1f))}}}}
