...
private fun LiveWatch(channel:Channel,vm:AppViewModel,full:Boolean,toggleFull:()->Unit) {
    ...
    LaunchedEffect(channel.id,date,retry,vm.sessionVersion){error=null;try{val d=vm.api.liveDetail(channel,date.toString());detail=d;if(uri.isBlank()||retry>0){replay=false;uri=d.uri;title=channel.title}}catch(e:Exception){if(e is CancellationException)throw e;error=safeError(e);loading=false}}
    LaunchedEffect(uri,retry){if(uri.isBlank())return@LaunchedEffect;if(!uri.startsWith("https://")){error="频道没有可用的安全播放源";loading=false;return@LaunchedEffect};player.setMediaItem(MediaItem.Builder().setUri(uri).setMediaMetadata(MediaMetadata.Builder().setTitle(title).build()).build());player.prepare();player.play()}
    Row(Modifier.fillMaxSize().padding(if(full)24.dp else 40.dp,12.dp),horizontalArrangement=Arrangement.spacedBy(22.dp)){
        Column(Modifier.weight(1f),verticalArrangement=Arrangement.spacedBy(12.dp)){
            Box(Modifier.fillMaxWidth().then(if(full)Modifier.weight(1f)else Modifier.aspectRatio(16f/9))){AndroidView(factory={PlayerView(it).apply{this.player=player;useController=false;isFocusable=false}},update={it.keepScreenOn=playing},modifier=Modifier.fillMaxSize());if(loading)Text("正在缓冲…",color=White,modifier=Modifier.background(Bg).padding(15.dp));error?.let{ErrorNotice(it){uri="";retry++}}}
            Text(if(replay)"回看 · $title" else "直播 · ${channel.title}",color=Green,fontSize=17.sp)
            LazyRow{item{TvAction(if(playing)"暂停"else"播放"){if(playing)player.pause()else player.play()}};if(replay){item{TvAction("后退30秒"){player.seekBack()}};item{TvAction("快进30秒"){player.seekForward()}}};item{TvAction(if(full)"退出全屏"else"全屏",onClick=toggleFull)};if(replay)item{TvAction("返回直播"){replay=false;title=channel.title;uri=detail?.uri?:""}}}
        }
