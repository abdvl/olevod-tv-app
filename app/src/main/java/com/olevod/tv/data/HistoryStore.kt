...
class HistoryStore(context:Context,private val scope:()->String):SQLiteOpenHelper(context,"history.db",null,2) {
    ...
    fun clearView(){_records.value=emptyList()}
    ...
    private fun read(account:String):List<WatchRecord> {val result=mutableListOf<WatchRecord>();readableDatabase.query("watches",null,"scope=?",arrayOf(account),null,null,"updated DESC").use{c->while(c.moveToNext()){runCatching{WatchRecord(MovieJson.decode(c.getString(c.getColumnIndexOrThrow("movie"))),c.getInt(c.getColumnIndexOrThrow("episode")),c.getLong(c.getColumnIndexOrThrow("position")),c.getLong(c.getColumnIndexOrThrow("duration")),c.getLong(c.getColumnIndexOrThrow("updated")))}.getOrNull()?.let{result+=it}}};return result}
    private fun publishCurrent(){val account=scope();val items=read(account);if(account==scope())_records.value=items}
    suspend fun load()=withContext(Dispatchers.IO){synchronized(this@HistoryStore){publishCurrent()}}
    suspend fun save(movie:Movie,episode:Int,position:Long,duration:Long,account:String=scope())=withContext(Dispatchers.IO){
        if(position<1000)return@withContext
        synchronized(this@HistoryStore){writableDatabase.insertWithOnConflict("watches",null,ContentValues().apply{put("scope",account);put("id",movie.id);put("movie",MovieJson.encode(movie));put("episode",episode);put("position",position);put("duration",duration);put("updated",System.currentTimeMillis())},SQLiteDatabase.CONFLICT_REPLACE);publishCurrent()}
    }
    suspend fun remove(id:Long?,account:String=scope())=withContext(Dispatchers.IO){synchronized(this@HistoryStore){writableDatabase.delete("watches",if(id==null)"scope=?" else "scope=? AND id=?",if(id==null)arrayOf(account) else arrayOf(account,id.toString()));publishCurrent()}}
}
