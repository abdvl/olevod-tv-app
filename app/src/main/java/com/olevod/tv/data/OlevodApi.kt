...
data class Episode(val index:Int,val title:String,val uri:String,val vip:Boolean)
...
class OlevodApi(
    ...
    suspend fun request(parts:List<String>,post:Any?=null):JSONObject {
        ...
        if(code!=0)throw ApiException(code,when(code){12,13,14,16->"登录已失效，请重新登录";else->"请求未完成（网站代码 $code）"})
        return json
    }
    ...
    fun image(path:String):String=when{path.startsWith("https://")->path;path.startsWith("//")->"https:$path";path.isBlank()->"";else->imageBase+"/"+path.trimStart('/')}
